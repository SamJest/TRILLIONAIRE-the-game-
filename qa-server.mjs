...
const homeR=await fetch(BASE+'/');const home=await homeR.text();assert(homeR.status===200&&home.includes('id="tr-race-launch"')&&home.includes('<title>TRILLIONAIRE — Mars Race</title>'),'launch UI missing from home');assert(!home.includes('COMP-1.2'),'rendered home contains stale COMP-1.2');assert(home.includes('COMP-1.3')&&home.includes('92% readiness')&&home.includes('75% system floor'),'rendered COMP-1.3 rules missing');console.log('PASS launch leaderboard surface and rendered COMP-1.3 rules');
