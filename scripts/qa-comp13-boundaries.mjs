...
function setState({ratio=.92,turn=16,crew=true,reliability=68,debt=0,launchRatio=null}={}){
  const systems={};
...
assert(html.includes('92% readiness · 75% system floor'),'visible COMP-1.3 objective missing');
...
setState({turn:15});
assert(!completion(),'turn 15 incorrectly wins');
setState({crew:false});
assert(!completion(),'missing crew incorrectly wins');
setState({ratio:.90});
assert(!completion(),'90% average readiness incorrectly wins');
setState({ratio:1,launchRatio:.74});
assert(!completion(),'74% system floor incorrectly wins');
...
setState({ratio:1,launchRatio:.75});
assert(completion(),'system floor exactly 75% should win with other gates met');

console.log('COMP13_BOUNDARY_QA '+JSON.stringify({ok:true,ruleset:'COMP-1.3',build:'v0.096',checks:10}));
